...
* **Deployment Stats Hydration:** It executes `refresh_redis_deployments` to recalculate the set of **assigned productive** computers for all active deployments. This ensures that the "Pending" counters in the v5 dashboard are accurate immediately after migration, restricted only to computers in operational status.

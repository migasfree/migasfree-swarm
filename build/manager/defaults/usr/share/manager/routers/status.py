...
async def service_stream(request: Request):
    ...
    logger.info(f'Service SSE client {client_id} connected. Total clients: {len(docker_monitor.sse_clients)}')

    async def event_generator():
        try:
            # Send initial state from cache
            cache_result = await docker_monitor.cache()
            logger.info(f'Client {client_id}: Sending initial cache with {len(cache_result)} services')
            ...
            for service_name, service_data in cache_result.items():
                ...
                logger.debug(f'Client {client_id}: Sending {service_name} = {status_obj["status"]} ({status_obj["running"]})')
            ...
            logger.info(f'Client {client_id}: Initial state sent, starting event stream')
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug(f'Client {client_id}: Broadcasting {event_data["event"]} event')
                    yield {
            ...
            logger.error(f'Client {client_id}: Error in event_generator: {e}', exc_info=True)
        finally:
            async with docker_monitor.client_id_lock:
                docker_monitor.sse_clients.pop(client_id, None)
            logger.info(f'Client {client_id}: Disconnected. Remaining: {len(docker_monitor.sse_clients)}')

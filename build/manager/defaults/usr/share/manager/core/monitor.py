...
class DockerSwarmMonitor:
    def __init__(self):
        ...
        try:
            ...
            if not swarm.get("ControlAvailable", False):
                logger.warning(
                    "Not running on a Swarm manager node - Docker monitoring disabled"
                )
                self.client = None
                return

            logger.info(
                f"Connected to Swarm cluster ID: {swarm.get('Cluster', {}).get('ID', 'unknown')[:12]}"
            )
            logger.info(
                f"Nodes: {swarm.get('Nodes', 0)} | Managers: {swarm.get('Managers', 0)}"
            )
        ...
        try:
            ...
            preparing = len(
                [
                    ...
                    for t in tasks
                    if t["Status"]["State"]
                    in ["preparing", "starting", "assigned", "accepted", "ready"]
                ]
            )
            failed = len(
                [
                    ...
                    for t in tasks
                    if t["Status"]["State"]
                    in ["failed", "rejected", "shutdown", "orphaned"]
                ]
            )
        ...
        async with self.cache_lock:
            ...
            if status_info:
                ...
                if "nodes" in status_info and status_info["nodes"]:
                    service_states_cache[service_name]["node"] = ", ".join(
                        status_info["nodes"]
                    )
                if "containers" in status_info and status_info["containers"]:
                    service_states_cache[service_name]["container"] = ", ".join(
                ...
                for service in services:
                    ...
                    async with self.lock:
                        ...
                        if prev_status is None:
                            ...
                            await self.update_service_cache(
                                service_name, current_status
                            )
        ...
        try:
            ...
            while self.running:
                event = await loop.run_in_executor(
                    None, lambda: next(event_stream, None)
                )
                if event is None:
    ...
    async def start(self):
        ...
        try:
            ...
            async with self.lock:
                for service in services:
                    if not service.name.startswith(
                        f"{STACK}_"
                    ) and not service.name.startswith("infra_"):
                        continue

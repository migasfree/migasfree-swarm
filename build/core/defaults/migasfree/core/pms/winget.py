...
set -x
cd %(path)s
cp /lib/x86_64-linux-gnu/libmsix1.so /lib/x86_64-linux-gnu/libmsix.so
makemsix pack -d %(path)s/source -p %(path)s/source.msix
rm -rf %(path)s/source
cp /lib/x86_64-linux-gnu/libmsix2.so /lib/x86_64-linux-gnu/libmsix.so
makemsix2 sign -p %(path)s/source.msix -c %(certificates_path)s/cert.pfx -cf PFX
if [ $? = 0 ]
then

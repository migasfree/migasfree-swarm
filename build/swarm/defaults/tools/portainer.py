...
def create_token(name, username, password):
    ...
    while True:
        ...
        try:
            response = session.post(f"http://portainer:9000/api/auth", json=payload)
    ...
    if response and 'jwt' in response.json():
        ...
        response = session.get(f'http://portainer:9000/api/users/me')    
        headers = session.headers
...
class PortainerAPI:
    ...
    def set_public_ip(self, ip):
        ...
        response = self.put(f"/endpoints/{self.endpoint_id}", payload)
        

    def custom_templates(self, payload):
        response = self.post(f"/custom_templates?method=string", payload)

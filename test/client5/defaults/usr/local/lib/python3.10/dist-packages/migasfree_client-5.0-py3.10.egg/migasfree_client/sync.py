    ...
    def _events(self):
        def to_prefix_dict(traits_list):
            ...
            for item in traits_list:
                if item['kind'] == 'J': # is a JSON
                    ret[item['prefix']].append(json.loads(item['value']))
                else:
        ...
        def to_env(content, prefix):
            ...
            for key in content.keys(): 
                if len(content[key]) == 1:
                    value = json.dumps(content[key][0])
                    ret += f"{prefix}{key}={value}\n"
                else:
                    value = ' '.join([f"'{json.dumps(item)}'" for item in content[key]])
                    ret += f'{prefix}{key}=({value})\n'
            return ret
